...
import 'package:clinicaltrac/view/p_evaluation/model/p_eval_list_model.dart';
...
class PEvaluationListScreen extends StatefulWidget {
  ...
  @override
  State<PEvaluationListScreen> createState() => _PEvaluationListScreenState();
}

class _PEvaluationListScreenState extends State<PEvaluationListScreen> {
  ...
  List<PEvaluationData> pEvaluationList = [];

  void getPEvaliList() async {
    if (pageNo == 1 || pageNo <= lastpage) {
      ...
      final DataResponseModel dataResponseModel =
          await dataService.getPEvalauionList(Request);

      // log(dataResponseModel.data.toString() + "SiteEval Data");
      PEvalListData pEvaluationResponse =
          PEvalListData.fromJson(dataResponseModel.data);

      lastpage =
          (int.parse(pEvaluationResponse.pager.totalRecords.toString()) / 10)
              .ceil();
      if (pEvaluationResponse.pager.totalRecords == '0') {
        setState(() {
          dataNotfound = true;
        });
      } else {
      ...
      setState(() {
        if (pageNo != 1) {
          pEvaluationList.addAll(pEvaluationResponse.data);
        } else {
          pEvaluationList = pEvaluationResponse.data;
        }
        isDataLoaded = false;
      });
    }
                            ...
                            child: Center(
                              child: Container(
                                ...
                                child: Padding(
                                    ...
                                    child: NoDataFoundWidget(
                                      title: "P Evaluations not available",
                                      imagePath: "assets/no_data_found.png",
                                    )),
                              ),
                            ),
                        ...
                        child: Padding(
                          ...
                          child: CupertinoScrollbar(
                            ...
                            child: ListView.builder(
                              shrinkWrap: true,
                              itemCount: pEvaluationList.length,
                              controller: _scrollController,
                              physics: const AlwaysScrollableScrollPhysics(),
                              itemBuilder: (context, index) {
                                evaluationDate = pEvaluationList[index]
                                            .evaluationDate !=
                                        ""
                                    ? dateConvert(
                                        pEvaluationList[index].evaluationDate)
                                    : '';
                                userId = int.parse(
                                ...
                                return CommonListContainerWidget(
                                  mainTitle:
                                      "Evaluation date : ${dateConvert("${pEvaluationList[index].evaluationDate}")}",
                                  subTitle3: "Hospital Site : ",
                                  title3: pEvaluationList[index].hospitalSites,
                                  subTitle20: "Overall Rating : ",
                                  title20: pEvaluationList[index].totalAvg,
                                  buttonTitle: "Edit",
